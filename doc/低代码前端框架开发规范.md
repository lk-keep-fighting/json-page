...
├── components/
│   ├── blocks/                   # 业务区块组件
│   │   ├── shared/               # 区块之间可复用的基础模块（表格、表单等）
│   │   │   └── data-table/       # 表格类能力：ActionBar、FilterBar、ActionFormDialog、AdminTable 等
│   │   ├── admin-table/
│   │   ├── data-management/
│   │   └── ...
│   ├── ui/                       # shadcn/ui 基础组件
...
- 当多个业务区块需要复用底层 UI 或交互逻辑时，应将实现放置在 `components/blocks/shared/` 目录下，根据功能域划分子目录（例如 `data-table/`）。
- 共享目录中的实现只沉淀可复用的通用能力，不直接耦合具体区块的配置结构；区块级组合逻辑继续保留在各自的业务目录中。
- 每个共享子目录需要提供 `index.ts` 统一导出内部能力，保证引用路径稳定、便于检索。
