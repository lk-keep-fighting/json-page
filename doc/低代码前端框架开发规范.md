...
| `type` | 额外字段 | 说明 |
| ------ | -------- | ---- |
| `"text"` / `"password"` | `placeholder`、`maxLength` | 单行输入，`password` 会对内容进行掩码 |
| `"number"` | `min`、`max`、`step` | 数值输入，提交时会自动转换为 `number` |
| `"textarea"` | `rows`、`maxLength` | 多行文本 |
| `"select"` | `options` | 下拉单选，提交时返回选项的 `value` |
| `"radio"` | `options` | 单选按钮组，更适合少量枚举项 |
| `"checkbox"` | `trueLabel`、`falseLabel` | 布尔开关，提交时返回 `true/false` |
| `"multi-select"` | `options`、`maxSelections` | 复选集合，返回选中值组成的数组 |
| `"date"` | `min`、`max` | 日期选择（`YYYY-MM-DD`）|
| `"time"` | `min`、`max` | 时间选择（`HH:mm`）|
| `"datetime"` | `min`、`max` | 日期时间输入（HTML `datetime-local`，`YYYY-MM-DDTHH:mm`）|

字段可声明 `required` 与 `defaultValue`。`defaultValue` 支持字面量、模板占位符以及数组（用于 `multi-select`）。`select` / `radio` 类型需保证默认值与某个选项的 `value` 一致，`checkbox` 建议直接传入布尔值。提交时的值会以 `formValues` 注入模板上下文。
